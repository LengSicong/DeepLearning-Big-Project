...
def load_data(filename, domain):
    ...
    for vid, records in data.items():
        ...
        for timestamp, sentence, ner, pos, dep_head, dep_tree in zip(records["timestamps"], records["sentences"], records["ner"], records["pos"], records["dep_head"], records["dep_tree"]):
            if domain == 'tacos':
            ...
            results.append((vid, start_time, end_time, duration, words, ner, pos, dep_head, dep_tree))
    return results
...
def generate_dataset(data, feature_length, word_dict, char_dict, max_pos_len, scope):
    ...
    for record in tqdm(data, total=len(data), desc="process {} data".format(scope)):
        video_id, start_time, end_time, duration, words, ner, pos, dep_head, dep_tree = record
        ...
        ner = ner[0:max_pos_len]
        pos = pos[0:max_pos_len]
        dep_head = dep_head[0:max_pos_len]
        dep_tree = dep_tree[0:max_pos_len]
        for word in words:
        ...
        example = {"vid": str(video_id), "s_time": float(start_time), "e_time": float(end_time),
                   ...
                   "num_units": int(num_units), "word_ids": word_ids, "char_ids": char_ids, "ner": ner, "pos": pos, "dep_head": dep_head, "dep_tree": dep_tree, "words": words}
...
def prepare_datasets(cfgs):
    ...
    # directory path
    data_dir = os.path.join('data', 'dataset', cfgs.task)
    feature_dir = os.path.join('data', 'features', cfgs.task)
    # load datasets
    train_data = load_data(os.path.join(data_dir, 'train.json'), cfgs.task)
    val_data = None if cfgs.task == 'charades' else load_data(os.path.join(data_dir, 'val.json'), cfgs.task)
    test_data = load_data(os.path.join(data_dir, 'test.json'), cfgs.task)
    ...
    for vid, length in feature_shapes.items():
        length = cfgs.max_pos_len if length > cfgs.max_pos_len else length
    ...
    # generate datasets
    train_set = generate_dataset(train_data, feature_lengths, word_dict, char_dict, cfgs.max_pos_len, "train")
    val_set = None if val_data is None else generate_dataset(val_data, feature_lengths, word_dict, char_dict,
                                                             cfgs.max_pos_len, "val")
    test_set = generate_dataset(test_data, feature_lengths, word_dict, char_dict, cfgs.max_pos_len, "test")
    # save datasets
    dataset = {'train_set': train_set, 'val_set': val_set, 'test_set': test_set, 'word_dict': word_dict,
               'char_dict': char_dict, 'word_vector': vectors}
    save_filename = os.path.join(cfgs.save_dir, '_'.join([cfgs.task, str(cfgs.max_pos_len)]) + '.pkl')
